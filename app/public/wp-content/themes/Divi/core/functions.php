...
function et_core_is_safe_mode_active( $product = false ) {
	static $safe_mode = null;

	// Return early if the safe mode status is already cached.
	if ( null !== $safe_mode ) {
		return $safe_mode;
	}

	// If checking against a particular product, check the usermeta for that product.
	if ( $product ) {
		$safe_mode = $product === get_user_meta( get_current_user_id(), '_et_support_center_safe_mode_product', true );
	} else {
		// Check global safe mode.
		$safe_mode = 'on' === get_user_meta( get_current_user_id(), '_et_support_center_safe_mode', true );
	}
