...
class ET_Core_Portability {
	...
	public function import_layout( $id, $layout, $chunk = 0, $onboarding = false ) {
		...
		if ( $result['ready'] ) {
			...
			$content = $onboarding ? wp_slash( D5BuilderConversion::maybeConvertContent( $content[0] ) ) : $content[0];
	...
	protected function _convert_and_import_d5_presets_from_onboarding( $new_presets_d4, $preset_prefix, $skip_default ) {
		...
		$converted_presets = D5BuilderConversion::maybe_convert_presets_data( $new_presets_d4 );
...
function et_core_portability_ajax_import() {
	...
	if ( ! $result = $portability->import() ) {
		...
		if ( $replace && $post_id > 0 && current_user_can( 'edit_post', $post_id ) ) {
				...
				$d5_content = $onboarding ? wp_slash( D5BuilderConversion::maybeConvertContent( $content ) ) : '';
				// phpcs:disable -- It's old code and phpcs is not working properly.
				wp_update_post(
					array(
					'ID'           => $post_id,
					'post_content' => $onboarding ? "<!-- wp:divi/placeholder -->{$d5_content}<!-- /wp:divi/placeholder -->" : $content,
					'meta_input'   => $onboarding ? ['_et_pb_use_divi_5' => 'on'] : [],
					)
				);
				// phpcs:enable
		}
