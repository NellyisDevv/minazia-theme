...
class MultiViewUtils {
	...
	public static function get_breakpoints_states_info(): MultiViewInfo {
		...
		$breakpoint_states  = self::get_breakpoints_states();
		$all_breakpoints    = array_keys( $breakpoint_states );
		$default_breakpoint = $all_breakpoints[0];
		$default_state      = $breakpoint_states[ $default_breakpoint ][0];
	...
	public static function get_breakpoints_states(): array {
		...
		foreach ( $breakpoints as $breakpoint ) {
			...
			foreach ( $states as $state ) {
				 ...
				 *
				 * - Desktop: value, hover, sticky
				 * - Tablet:  value
				 * - Phone:   value
				 */
				$is_enable_state_default = 'desktop' === $breakpoint || 'value' === $state;
		...
		foreach ( $values as $key => $attr ) {
			...
			foreach ( $attr as $breakpoint => $states_values ) {
				...
				foreach ( $states as $state ) {
					...
					$merged[ $breakpoint ][ $state ][ $key ] = ModuleUtils::get_attr_value(
						[
							'attr'       => $attr,
		...
		foreach ( $breakpoint_states as $breakpoint => $states ) {
			...
			foreach ( $states as $state ) {
				...
				if ( $is_default_breakpoint && $is_default_state ) {
					...
					$state_value = ModuleUtils::get_attr_value(
						[
							'attr'       => $value,
							'breakpoint' => $breakpoint,
							'state'      => $state,
					...
					if ( $unique ) {
						if ( ! $is_default_state && isset( $normalized[ $breakpoint ][ $default_state ] ) ) {
							...
							$value_to_compare = $normalized[ $prev_breakpoint ][ $default_state ] ?? $normalized[ $default_breakpoint ][ $default_state ];
						}
	 ...
	 * @return array
	 */
	public static function unique_data( array $data ):array {
		$unique_data = [];
		...
		foreach ( $data as $key => $value ) {
			...
			switch ( $breakpoint ) {
				case 'phone':
					if ( 'value' === $state ) {
						$value_inherit = $data['tablet'] ?? $data['desktop'] ?? null;
					} else {
						$value_inherit = $data['phone'] ?? $data['tablet'] ?? $data['desktop'] ?? null;
					}
				...
				case 'tablet':
					if ( 'value' === $state ) {
						$value_inherit = $data['desktop'] ?? null;
					} else {
						$value_inherit = $data['tablet'] ?? $data['desktop'] ?? null;
					}
				...
				case 'desktop':
					$skip = 'value' === $state ? false : $data['desktop'] === $value;
					break;
