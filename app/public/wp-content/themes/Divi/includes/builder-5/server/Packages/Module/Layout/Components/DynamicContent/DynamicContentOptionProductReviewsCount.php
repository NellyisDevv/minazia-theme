<?php
/**
 * Module: DynamicContentOptionProductReviewsCount class.
 *
 * @package Divi
 * @since ??
 */
 ...
 * @since ??
 */
class DynamicContentOptionProductReviewsCount extends DynamicContentOptionBase implements DynamicContentOptionInterface {

	/**
	 * Get the name of the product reviews count option.
	 *
	 * @since ??
	 *
	 * @return string The name of the product reviews count option.
	 */
	...
	/**
	 * Get the label for the product reviews count option.
	 *
	 * This function retrieves the localized label for the product reviews count option,
	 * which is used to describe the product reviews count in user interfaces.
	 *
	 * @since ??
	 *
	 * @return string The label for the product reviews count option.
	 */
	public function get_label(): string {
		return esc_html__( 'Product Reviews Count', 'et_builder' );
	}

	/**
	 * Callback for registering product reviews count option .
	 *
	 * This function is a callback for the `divi_module_dynamic_content_options` filter .
	 * This function is used to register options for product reviews count by adding them to the options array passed to the function .
	...
	/**
	 * Render callback for product reviews count option.
	 *
	 * Retrieves the value of product reviews count option based on the provided arguments and settings.
	 * This is a callback for `divi_module_dynamic_content_resolved_value` filter.
	 *
	 * @since ??
	 *
	 * @param mixed $value     The current value of the product reviews count option.
	 * @param array $data_args {
	 *     Optional. An array of arguments for retrieving the product reviews count.
	 *     Default `[]`.
	 *
	 ...
	 * }
	 *
	 * @return string The formatted value of the product reviews count option.
	 *
	 * @example:
	 * ```php
	 ...
	 *        'product_title' => 'Product reviews count',
	 *      ],
	 *  ] );
	 * ```
	 */
	public function render_callback( $value, array $data_args = [] ): string {
		...
		if ( $post ) {
			...
			if ( $dynamic_product ) {
				$value = $dynamic_product->get_review_count();
