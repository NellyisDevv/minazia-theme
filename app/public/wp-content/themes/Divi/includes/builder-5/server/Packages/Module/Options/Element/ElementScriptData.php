...
class ElementScriptData {
	 ...
	 * {@link /docs/builder-api/js/module/use-element-script-data useElementScriptData} in
	 * `@divi/module` package.
	 *
	 * @since ??
	 *
	...
	public static function set( array $args ): void {
		 ...
		 * module (`divi/accordion-item`) even if `$attrs['scroll']` is empty. Because scroll effects might need
		 * to be inherited from the parent module (divi/accordion) when the parent has `gridMotion` option enabled.
		 */
		if ( ! empty( $attrs['scroll'] ) || false !== strpos( $id, 'divi/accordion-item' ) ) {
