...
class TransitionUtils {
	...
	public static function compose_transition_css_properties( string $mode = '', array $attrs = [] ): array {
		...
		foreach ( $attrs as $attrs_key => $attr_value ) {
			if ( is_array( $attr_value ) && ! empty( $attr_value ) ) {
				foreach ( $attr_value as $module_attr_key => $module_attr ) {
					if ( 'desktop' === $module_attr_key && ! empty( $module_attr ) ) {
						if ( 'background' === $attrs_key ) {
							$background_transition_css = self::get_transition_background_properties( $module_attr, $mode );
							if ( ! empty( $background_transition_css ) ) {
								$css_properties[] = $background_transition_css;
							}
						} elseif ( 'border' === $attrs_key ) {
							$border_transition_css = self::get_transition_border_properties( $module_attr, $mode );
							if ( ! empty( $border_transition_css ) ) {
								$css_properties[] = $border_transition_css;
							}
						} elseif ( 'boxShadow' === $attrs_key ) {
							$box_shadow_transition_css = self::get_transition_box_shadow_properties( $module_attr, $mode );
							if ( ! empty( $box_shadow_transition_css ) ) {
								$css_properties[] = $box_shadow_transition_css;
							}
						} elseif ( 'filters' === $attrs_key ) {
							$filter_transition_css = self::get_transition_filters_properties( $module_attr, $mode );
							if ( ! empty( $filter_transition_css ) ) {
								$css_properties[] = $filter_transition_css;
							}
						} elseif ( 'position' === $attrs_key ) {
							$position_transition_css = self::get_transition_position_properties( $module_attr, $mode );
							if ( ! empty( $position_transition_css ) ) {
								$css_properties[] = $position_transition_css;
							}
						} elseif ( 'sizing' === $attrs_key ) {
							$sizing_transition_css = self::get_transition_sizing_properties( $module_attr, $mode );
							if ( ! empty( $sizing_transition_css ) ) {
								$css_properties[] = $sizing_transition_css;
							}
						} elseif ( 'spacing' === $attrs_key ) {
							$spacing_transition_css = self::get_transition_spacing_properties( $module_attr, $mode );
							if ( ! empty( $spacing_transition_css ) ) {
								$css_properties[] = $spacing_transition_css;
							}
						} elseif ( 'transform' === $attrs_key ) {
							$transform_transition_css = self::get_transition_transform_properties( $module_attr, $mode );
							if ( ! empty( $transform_transition_css ) ) {
								$css_properties[] = $transform_transition_css;
							}
						} elseif ( 'zIndex' === $attrs_key ) {
							$zindex_transition_css = self::get_transition_zindex_properties( $module_attr, $mode );
							if ( ! empty( $zindex_transition_css ) ) {
								$css_properties[] = $zindex_transition_css;
							}
						} elseif ( 'font' === $attrs_key ) {
							$font_transition_properties = self::get_transition_font_properties( $module_attr, $mode );
							if ( ! empty( $font_transition_properties ) ) {
								$css_properties[] = $font_transition_properties;
							}
						} elseif ( 'textShadow' === $attrs_key ) {
							$text_shadow_transition_properties = self::get_transition_text_shadow_properties( $module_attr, $mode );
							if ( ! empty( $text_shadow_transition_properties ) ) {
								$css_properties[] = $text_shadow_transition_properties;
							}
						} elseif ( 'icon' === $attrs_key ) {
							$icon_transition_properties = self::get_transition_icon_properties( $module_attr, $mode );
							if ( ! empty( $icon_transition_properties ) ) {
								$css_properties[] = $icon_transition_properties;
							}
						}
					} elseif ( 'font' === $module_attr_key && ! empty( $module_attr ) ) {
						foreach ( $module_attr as $font_module_attr_key => $font_module_attr_value ) {
							if ( 'desktop' === $font_module_attr_key && ! empty( $font_module_attr_value ) ) {
								$font_transition_properties = self::get_transition_font_properties( $font_module_attr_value, $mode );
								if ( ! empty( $font_transition_properties ) ) {
									$css_properties[] = $font_transition_properties;
								}
							}
						}
					} elseif ( 'textShadow' === $module_attr_key && ! empty( $module_attr ) ) {
						foreach ( $module_attr as $text_shadow_module_attr_key => $text_shadow_module_attr_value ) {
							if ( 'desktop' === $text_shadow_module_attr_key && ! empty( $text_shadow_module_attr_value ) ) {
								$text_shadow_transition_properties = self::get_transition_text_shadow_properties( $text_shadow_module_attr_value, $mode );
								if ( ! empty( $text_shadow_transition_properties ) ) {
									$css_properties[] = $text_shadow_transition_properties;
								}
							}
