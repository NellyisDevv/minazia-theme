...
class BarCountersItemModule implements DependencyInterface {
			...
			[
				...
				'children'   => [
					...
					'valueResolver' => function( $value ) {
						...
						$percent             = $percent ?? '0';
						$percent_with_symbol = $percent . '%';

						return 'on' === $use_percentages ? $percent_with_symbol : $percent;
					},
				],
			]
		);
	...
	public static function bar_width_style_declaration( array $params ): string {
		...
		if ( isset( $params['attrValue'] ) ) {
			$style_declarations->add( 'width', $params['attrValue'] . '%' );
		}
	...
	public static function bar_overlay_color_style_declaration( array $params ): string {
		...
		if ( isset( $params['attrValue']['color'] ) ) {
			$style_declarations->add( 'color', $params['attrValue']['color'] );
		}
							...
							'styleProps' => [
								'advancedStyles' => [
									[
										'componentName' => 'divi/common',
										'props'         => [
											'selector' => "{$args['orderClass']} .et_pb_counter_amount",
											'attr'     => $attrs['barProgress']['innerContent'] ?? [],
									...
									[
										'componentName' => 'divi/common',
										'props'         => [
											'selector' => "{$args['orderClass']} .et_pb_counter_amount.overlay",
