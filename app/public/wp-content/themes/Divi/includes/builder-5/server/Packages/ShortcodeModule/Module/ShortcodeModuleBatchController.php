...
class ShortcodeModuleBatchController extends RESTController {
	...
	public static function index( WP_REST_Request $request ): WP_REST_Response {
			...
			foreach ( $batch_data as $item_data ) {
				...
				try {
					...
					$item_response['error'] = sprintf(
						__( 'Error when rendering Shortcode module with shortcode name %1$s and shortcode content %2$s', 'et_builder' ),
				...
				/**
				 * Return format for each requests of `batchFetch` call.
				 *
				 * Set of `data` and `response` for each requests in the batch. Both of `data`
				 * and `response` properties are needed as unique identifier and resolved value
				 * respectively for the log request. Both of them are mandatory to have.
				 *
				 * @var array $item_result {
				 *  @type array $data     Request `data` from `loggedFetch` options method.
				 *  @type array $response Request `response` for `loggedFetch` call. The value
				 *                        can be defined with any value.
				 * }
				 *
				 * @example
				 *
				 ...
				 *    ],
				 *    'response' => [
				 *      'html' => '<div class="et_pb_registered_module">Registered Module</div>',
				 *    ],
				 * ]
				 */
				$item_result = [
				...
				/**
				 * Collection of `data` and `response` for each requests in the batch.
				 *
				 * @var $item_result[] $result
				 */
				$result[] = $item_result;
			}
		}
		...
		/**
		 * Return format for `batchFetch` call.
		 *
		 * The `result` property is mandatory because it's needed as property to save the
		 * collections of `data` and `response` for each requests in the batch.
		 *
		 * @see $result
		 *
		 * @var array $response {
		 *  @type array $result Collection of `data` and `response` for each requests.
		 * }
		 */
		$response = [
			'result' => $result,
