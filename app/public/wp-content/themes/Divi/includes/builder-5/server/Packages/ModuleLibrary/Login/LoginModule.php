...
class LoginModule implements DependencyInterface {
	...
	public static function render_logged_in_as_content( string $content, string $current_page_redirect ): string {
		$content = FormattingUtility::maybe_wpautop( $content );
		...
		$content .= sprintf( esc_html__( 'Logged in as %1$s', 'et_builder' ), esc_html( $current_user->display_name ) );
		...
		$content .= ' ' . HTMLUtility::render(
			[
				...
				'children'          => esc_html__( 'Log out', 'et_builder' ),
	...
	/**
	 * Render the content with a specified current page redirect.
	 *
	 * This method is used to render the content with a specific current page redirect value.
	 * It creates a div element with the class "et_pb_newsletter_description_content" and passes it to the
	 * `HTMLUtility::render()` method along with the provided content and current page redirect value.
	 * The content is sanitized using the "et_core_esc_previously" sanitizer before rendering.
	 *
	 * @since ??
	 *
	 * @param string $content               The content to render.
	 * @param string $current_page_redirect Optional. The current page redirect setting. Default `'off'`.
	 *
	 * @return string The rendered content.
	 */
	public static function render_content( string $content, string $current_page_redirect = 'off' ): string {
		return HTMLUtility::render(
			[
				...
				'children'          => self::render_logged_in_as_content( $content, $current_page_redirect ),
				'childrenSanitizer' => 'et_core_esc_previously', // Content has been sanitized during the saving post content process.
			]
		);
	}
			...
			[
					...
					[
						...
						'data'          => MultiViewUtils::merge_values(
							[
								...
								'currentPageRedirect' => $attrs['module']['advanced']['currentPageRedirect'] ?? [],
							]
						),
						'valueResolver' => function ( $value ) {
							return LoginModule::render_content( $value['content'] ?? '', $value['currentPageRedirect'] ?? 'off' );
						},
				...
				'setVisibility' => [
					[
						...
						'valueResolver' => function ( $value ) {
							return ! ( '' === ( $value['title'] ?? '' ) && '' === LoginModule::render_logged_in_as_content( $value['content'] ?? '', $value['currentPageRedirect'] ?? 'off' ) ) ? 'visible' : 'hidden';
						},
					],
				],
			]
	 ...
	 *
	 * This function is equivalent of JS const:
	 * {@link /docs/builder-api/js/module-library/generate-default-attrs cssFields} located in `@divi/module-library`.
	 *
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements ): string {
		...
		if ( ! is_user_logged_in() || is_customize_preview() || is_et_pb_preview() ) {
			...
			$button_element = ButtonComponent::component(
				[
					'name'         => 'et_builder_submit_button',
					'className'    => 'et_pb_newsletter_button',
					'hasWrapper'   => false,
					'type'         => 'button',
					'innerContent' => [
						'desktop' => [
							'value' => [
								'text' => __( 'Login', 'et_builder' ),
							],
						],
					],
					'buttonAttr'   => $attrs['button']['decoration']['button'] ?? [],
				]
			);
			...
			$user_login_input = HTMLUtility::render(
				[
					'tag'        => 'input',
					'attributes' => [
			...
			$user_pass_input = HTMLUtility::render(
				[
					'tag'        => 'input',
					'attributes' => [
			...
			$lost_password_url = HTMLUtility::render(
				[
					...
					'children'          => esc_html__( 'Forgot your password?', 'et_builder' ),
					'childrenSanitizer' => 'et_core_esc_previously',
				]
			);
		...
		$title_and_content = $is_render_title || $is_render_content ? HTMLUtility::render(
			[
				...
				'children'          => [
					...
					$is_render_content ? self::render_content( $content, $current_page_redirect ) : '',
				],
				'childrenSanitizer' => 'et_core_esc_previously',
			]
		) : '';
