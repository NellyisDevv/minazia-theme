...
class AudioModule implements DependencyInterface {
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements, array $default_printed_style_attrs ): string {
		...
		$image_url = $has_image_url ? HTMLUtility::render(
			[
				...
				'attributes'        => [
					...
					'style' => [
						'background-image' => sprintf( 'url(%1$s)', $image_link ),
					],
				],
		...
		$meta = self::render_element_meta(
			$attrs['artistName']['innerContent']['desktop']['value'] ?? '',
			$attrs['albumName']['innerContent']['desktop']['value'] ?? ''
		);
				...
				'setContent'    => [
					[
						...
						'valueResolver' => function ( $value ) {
							...
							return AudioModule::render_element_meta( $artist_name, $album_name, true );
						},
	 ...
	 * @since ??
	 *
	 * @param string $artist_name Artist name for the audio element.
	 * @param string $album_name  Album name for the audio element.
	 * @param bool   $no_wrapper  Flag to indicate whether or not to include a paragraph tag with a class.
	 *
	 ...
	 * $no_wrapper  = false;
	 * $html        = RenderElementMetaTrait::render_element_meta( $artist_name, $album_name, $no_wrapper );
	 * ```
	 */
	public static function render_element_meta( string $artist_name, string $album_name, bool $no_wrapper = false ): string {
		$items = [];
