...
class DynamicContentOptionProductPrice extends DynamicContentOptionBase implements DynamicContentOptionInterface {
	...
	public function get_label(): string {
		return esc_html__( 'Product Price', 'et_builder' );
	}
	...
	public function render_callback( $value, array $data_args = [] ): string {
		...
		if ( $post ) {
			...
			if ( $dynamic_product ) {
				// TODO feat(D5, Woo Modules): Replace it later once the Woo Price module
				// is implemented in D5.
				// @see https://github.com/elegantthemes/Divi/issues/25712.
				$value = \ET_Builder_Module_Woocommerce_Price::get_price(
					[
