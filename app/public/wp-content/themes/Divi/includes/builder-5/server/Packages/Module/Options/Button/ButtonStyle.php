...
class ButtonStyle {
	 ...
	 * {@link /docs/builder-api/js/module/button-style ButtonStyle} in
	 * `@divi/module` package.
	 *
	 * @since ??
	 *
			...
			$children_statements = Utils::get_statements(
				[
					...
					'selectorFunction'        => function( $params ) {
						...
						$main_placement    = $attr['desktop']['value']['icon']['placement'] ?? $default_placement;
						$current_placement = $is_main
							? $main_placement
							: (
								$attr[ $breakpoint ][ $state ]['icon']['placement'] ?? $main_placement
							);
