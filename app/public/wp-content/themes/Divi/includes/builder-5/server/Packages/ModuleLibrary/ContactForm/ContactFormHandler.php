...
class ContactFormHandler {
	...
	public function process():void {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( $_POST[ '_wpnonce-et-pb-contact-form-submitted-' . $order_index ] ), 'et-pb-contact-form-submit-' . $order_index ) ) {
			$this->_add_error( 'invalid_nonce', esc_html__( 'Nonce verification failed. Please refresh the page and try again.', 'et_builder' ) );
			return;
		}
		...
		if ( 'on' === $use_basic_captcha && 'off' === $use_spam_service ) {
			...
			if ( ! $captcha_answer || ( (int) $first_digit + (int) $second_digit ) !== (int) $captcha_answer ) {
				$this->_add_error( 'invalid_captcha', esc_html__( 'Make sure you entered the correct captcha.', 'et_builder' ) );
			}
			...
			if ( empty( $_POST['token'] ) || ! SpamProtectionService::validate_token( $provider, $account, $min_score ) ) {
				$this->_add_error( 'spam_submission', esc_html__( 'You must be a human to submit this form.', 'et_builder' ) );
			}
		}
		...
		foreach ( $fields as $field ) {
			...
			if ( ! $field_label ) {
				$field_label = __( 'New Field', 'et_builder' );
			}
		...
		if ( ! $this->_error->has_errors() ) {
			...
			if ( $message_pattern ) {
				$message = ContactFormUtils::build_message_by_template( $this->_fields, $message_pattern );
			} else {
	...
	public function validate_fields():void {
		foreach ( $this->_fields_raw as $field_id => $field ) {
			...
			if ( $field['isRequired'] && '' === $field_value ) {
				$this->_add_error( 'field_required_' . $field_id, sprintf( esc_html__( '%s: The field is required.', 'et_builder' ), $field_label ) );
				continue;
			}
				...
				switch ( $field_type ) {
					case 'email':
						if ( ! is_email( $field_value ) ) {
							$this->_add_error( 'value_not_email_' . $field_id, sprintf( esc_html__( '%s: The value must be a valid email address.', 'et_builder' ), $field_label ) );
							continue 2;
						}
						break;
					...
					case 'radio':
					case 'select':
						if ( ! in_array( $field_value, $field['options'], true ) ) {
							$this->_add_error( 'value_not_exists_' . $field_id, sprintf( esc_html__( '%s: The selected value is invalid.', 'et_builder' ), $field_label ) );
							continue 2;
						}
						break;
					...
					case 'checkbox':
						...
						foreach ( $parts as $part ) {
							if ( ! in_array( $part, $field['options'], true ) ) {
								$this->_add_error( 'value_not_exists_' . $field_id, sprintf( esc_html__( '%s: The selected value is not exist.', 'et_builder' ), $field_label ) );
								continue 3;
							}
						}
						break;
					...
					case 'input':
						...
						if ( 'letters' === $allowed_symbols && ! preg_match( '/^[a-zA-Z\s\-]+$/', $field_value ) ) {
							$this->_add_error( 'value_not_letters_' . $field_id, sprintf( esc_html__( '%s: The value may only contain letters.', 'et_builder' ), $field_label ) );
							continue 2;
						}

						if ( 'numbers' === $allowed_symbols && ! preg_match( '/^[0-9\s\-]+$/', $field_value ) ) {
							$this->_add_error( 'value_not_numbers_' . $field_id, sprintf( esc_html__( '%s: The value may only contain numbers.', 'et_builder' ), $field_label ) );
							continue 2;
						}

						if ( 'alphanumeric' === $allowed_symbols && ! preg_match( '/^[\w\s\-]+$/', $field_value ) ) {
							$this->_add_error( 'value_not_letters_and_numbers_' . $field_id, sprintf( esc_html__( '%s: The value may only contain letters and numbers.', 'et_builder' ), $field_label ) );
							continue 2;
						}
						...
						if ( $max_length > 0 && strlen( $field_value ) > $max_length ) {
							$this->_add_error( 'value_max_length_' . $field_id, sprintf( esc_html__( '%1$s: The value may not be greater than %2$d characters.', 'et_builder' ), $field_label, $max_length ) );
							continue 2;
						}
						...
						if ( $min_length > 0 && strlen( $field_value ) < $min_length ) {
							$this->_add_error( 'value_min_length_' . $field_id, sprintf( esc_html__( '%1$s: The value must be at least %2$d characters.', 'et_builder' ), $field_label, $min_length ) );
							continue 2;
						}
						break;
