...
class StaticCSS implements DependencyInterface {
	...
	public static function setup_styles_manager( int $post_id = 0 ) {
		...
		$forced_in_footer = 'on' === et_get_option( 'et_pb_css_in_footer', 'off' );
	...
	public static function style_output( array $params ) {
		 ...
		 * - Above the fold styles is kept on `default` style type.
		 * - Below the fold styles is kept on `deferred` style type.
		 *
		 * When critical CSS should not be generated, all styles are kept on `default` style type.
		 */
		if ( $should_generate_critical_css ) {
			$critical = Style::render( 'default', 'preset' ) . Style::render( 'default', 'module' );
			$styles   = Style::render( 'deferred', 'preset' ) . Style::render( 'deferred', 'module' );
		} else {
			$styles = Style::render( 'default', 'preset' ) . Style::render( 'default', 'module' );
		}
