...
class ModuleElements {
	 ...
	 *     @type GlobalPresetItem    $presetItem     Optional. The global preset item. Default `null`.
	 * }
	 */
	public function __construct( array $args = [] ) {
		...
		$preset_item = $args['presetItem'] ?? null;

		if ( $preset_item instanceof GlobalPresetItem ) {
			$this->preset_item = $preset_item;
		} else {
			$this->preset_item = GlobalPreset::get_item( $this->name, $this->module_attrs );
		}
	}
	...
	public function render( array $args ): string {
		...
		switch ( $element_type ) {
			case 'heading':
				...
				$tag_name = $element_attr['decoration']['font']['font']['desktop']['value']['headingLevel'] ?? $tag_name;
				break;
	...
	public function script_data( array $args ): void {
		...
		$decoration_attr = $this->module_attrs[ $attr_name ]['decoration'] ?? [];
	...
	public function style( array $args ) {
		...
		if ( $is_preset_style_group ) {
			$item_preset_attrs = $this->preset_item->get_data_attrs();
			$decoration_attr   = $item_preset_attrs[ $attr_name ]['decoration'] ?? [];
		}
	...
	public function style_components( array $args ) {
		...
		$preset_item_attrs      = $this->preset_item->get_data_attrs() ?? [];
		$preset_decoration_attr = $preset_item_attrs[ $attr_name ]['decoration'] ?? [];

		// Merge preset decoration attributes.
		$decoration_attr = array_replace_recursive( $preset_decoration_attr, $decoration_attr );
	...
	/**
	 * Get preset item class name.
	 *
	 * @since ??
	 *
	 * @return string The preset item class name.
	 */
	public function get_preset_item_class_name() {
		return ModuleUtils::get_preset_class_name( $this->module_metadata->name, $this->module_attrs ?? [] );
	}
