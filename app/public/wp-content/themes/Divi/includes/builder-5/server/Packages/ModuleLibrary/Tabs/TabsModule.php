...
class TabsModule implements DependencyInterface {
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements ): string {
			...
			$title = $child_element->render(
				[
					...
					'valueResolver'     => function ( $value ) {
						return '' === ( $value ?? '' ) ? esc_html__( 'Tab', 'et_builder' ) : $value;
					},
		...
		$tabs_controls_rendered = ! empty( $tabs_controls_rendered ) ? HTMLUtility::render(
			[
				'tag'               => 'ul',
				'attributes'        => [
					'class' => HTMLUtility::classnames(
		...
		$all_tabs_rendered = ! empty( $content ) ? HTMLUtility::render(
			[
				'tag'               => 'div',
				'attributes'        => [
					'class' => HTMLUtility::classnames(
		...
		return Module::render(
			[
				...
				) . $tabs_controls_rendered . $all_tabs_rendered,
			]
		);
	}

	/**
	 * Loads `TabsModule` and registers Front-End render callback and REST API Endpoints.
	 *
	 * @since ??
	 *
	 * @return void
