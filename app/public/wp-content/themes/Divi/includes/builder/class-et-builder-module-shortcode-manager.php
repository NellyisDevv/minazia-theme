...
class ET_Builder_Module_Shortcode_Manager {
	...
	private static function _should_register_shortcodes() {
		...
		// dont load builder files when in REST API request.
		...
		if ( $is_vb_enabled && $has_page_id ) {
			...
			// Only load these when this is a request for the VB from GB.
			$should_register_shortcodes = true;
			return $should_register_shortcodes;
		}
		...
		if ( $is_vb_enabled && $is_app_window ) {
			...
			// Return false if the post contains no Woo modules "[et_pb_woo" or "[et_pb_shop"
			global $post;
			if ( isset( $post ) && ( false !== strpos( $post->post_content, '[et_pb_wc' ) && false !== strpos( $post->post_content, '[et_pb_shop' ) ) ) {
				return false;
			}
