...
function et_theme_builder_frontend_override_template( $template ) {
	$is_template_preview = isset( $_GET['et_tb_preview'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'et_theme_builder_template_preview' ); // phpcs:ignore ET.Sniffs.ValidatedSanitizedInput.InputNotSanitized -- wp_verify_nonce() function does sanitation.
	$layouts             = et_theme_builder_get_template_layouts();
	$page_template       = locate_template( 'page.php' );
	$override_header     = ! $is_template_preview && et_theme_builder_overrides_layout( ET_THEME_BUILDER_HEADER_LAYOUT_POST_TYPE );
	$override_body       = ! $is_template_preview && et_theme_builder_overrides_layout( ET_THEME_BUILDER_BODY_LAYOUT_POST_TYPE );
	$override_footer     = ! $is_template_preview && et_theme_builder_overrides_layout( ET_THEME_BUILDER_FOOTER_LAYOUT_POST_TYPE );
	...
	if ( $override_header || $override_footer || $is_template_preview ) {
		// wp-version >= 5.2
