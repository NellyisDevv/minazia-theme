...
class ET_Builder_Settings {
	...
	protected static function _get_builder_settings_fields() {
		$builder_settings_fields = array(
			...
			'et_force_enable_shortcode_framework' => array(
				...
				'description'     => esc_html__( 'Force the Legacy Divi 4 shortcode framework to load on every page regardless if shortcodes are used on that page. The default value for this setting is off, where the shortcode framework will only load on a per-page, as needed, basis. Note: This enabling this option will significantly decrease front-end performance.', 'et_builder' ),
				'options'         => array(
					'on'  => __( 'On', 'et_builder' ),
					'off' => __( 'Off', 'et_builder' ),
				),
	...
	protected static function _get_page_settings_values( $post_id ) {
		...
		// If D5 is enabled, conditionally convert global colors option data format.
		if ( et_builder_d5_enabled() ) {
			// Only run conversion if the global colors flag is enabled.
			// @see https://github.com/elegantthemes/Divi/issues/25789.
			if ( defined( 'ET_BUILDER_5_EXPERIMENTS_GLOBAL_DATA_COLORS' ) && ET_BUILDER_5_EXPERIMENTS_GLOBAL_DATA_COLORS ) {
